...
import {profileNavigationOptions, QuestionnaireNavigationOptions} from "../navigators/options";
...
import {addIcon, barcode, check, checkSelected, closeIcon, radioSelected, radioUnselected} from "../utils/images";
...
import {saveQuestions, saveUuidValues, setAnswers, setRequiredQuestions, setSync} from "../actions/questions";
...
class QuestionnaireScene extends Component {
        ...
        for (const answer of object.answer_option) {
            ...
            options.push(
                <TouchableOpacity
                    ...
                    style={[checkStyle.checkElementTouch, optionIndex > 0 ? {marginTop: 16} : {}]} key={answer.id}>
                    <Image source={icon}/>
    ...
    renderBarcodeInput(question, index, group) {
        ...
        const borderStyle = Platform.OS === "ios" ? {borderBottomWidth: 1, borderBottomColor: "#bcbbc1"} : {};
        return (
        ...
        for (const require of this.state.required) {
            if (this.state.answers.has(this.state.visitUuid + '_' + require) === false) {
                return Alert.alert(I18n.t("questions.haveRequireQuestions"), I18n.t("questions.requireText"), [
                    {text: I18n.t("questions.ok"), onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
                ]);
            }
        }
        this.props.saveUuidValues(this.state.uuidValues);
