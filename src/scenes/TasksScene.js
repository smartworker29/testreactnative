...
import {addIcon, distanceIcon, fiveImage, pinIcon, tasksImage} from "../utils/images";
import {requestShopById, setFavorite} from "../actions/shops";
import {is} from "immutable"
import {distance, getCoordinates} from "../utils/util";
...
class TasksScene extends Component {
    ...
    shopHeader = () => {
        ...
        return (
            <View style={styles.header}>
                <View style={styles.headerMain}>
                    {(logo) ? <CachedImage style={styles.shopLogo} source={{uri: logo}} resizeMode="contain"/> : null}
                ...
                {this.props.list.count() !== 0 ? <Text style={styles.tasksLabel}>{I18n.t("tabBar.tasks")}</Text> : null}
            </View>
        )
    };
