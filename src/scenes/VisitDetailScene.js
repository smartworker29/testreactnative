...
export class VisitDetailScene extends Component {
  ...
  renderShopDetail(visit) {
    ...
    return (
      <View style={styles.topRow}>
        {(logo) ?
          <CachedImage style={styles.icon} source={{uri: logo}} resizeMode="contain"/> : null}
    ...
    return (
      <View>
        ...
        <TouchableOpacity style={[styles.QuestionnaireButton, borderColor]}
                          onPress={() => this.goToQuestionnaire(task.question_group, visitUuid)}>
          <Text
            style={[styles.QuestionnaireText, textColor]}>{I18n.t("questions.answerButton")}</Text>
        </TouchableOpacity>
  ...
  renderAnswer(questionUUID, visitUuid) {
    ...
    if (answer.select_one !== undefined) {
      return <Text
        style={stylesQuestions.answerValue}>{this.props.uuidValues.get(answer.select_one)}</Text>
    }
      ...
      case 'NEGATIVE' :
        moderationRow = (
          <View style={styles.infoStatusRow}>
            ...
            <Text
              style={[styles.statusText, styles.red]}>{I18n.t('visitDetail.moderationReject')}</Text>
          </View>
        );
        break;
      case 'POSITIVE' :
        moderationRow = (
          <View style={styles.infoStatusRow}>
            ...
            <Text
              style={[styles.statusText, styles.green]}>{I18n.t('visitDetail.moderationDone')}</Text>
          </View>
        );
        break
    }
  ...
  backHandler = () => {
    this.timeoutBackHandler = setTimeout(() => {
      this._isMounted && this.setState({showBtn: true})
    }, 1500);
  };
  ...
  goToPhoto = async (id) => {
    ...
    if (allowAction("goToPhoto")) {
      this._isMounted && this.setState({showBtn: false}, () => {
    ...
    return (
      <View style={{flex: 1}}>
          ...
          <View style={styles.alertArea}>
            <View style={styles.pictureWindow} elevate={5}>
              ...
              <Image source={{uri: this.state.pictureUrl}} style={{width: modalWidth, height: 300}}
                     resizeMode="contain" onLoadEnd={() => this._isMounted && this.setState({pictureLoading: false})}
                     onLoadStart={() => this._isMounted && this.setState({pictureLoading: true})}/>
              <View style={{marginTop: 30}}>
                <Button
                  onPress={() => this._isMounted && this.setState({showPicture: false})}
        ...
        {allowPhotoByHours && this.state.showBtn ?
          <GradientButton icon={cameraIcon} style={styles.takePhotoBtn}
                          text={I18n.t('photo.makePhoto')}
